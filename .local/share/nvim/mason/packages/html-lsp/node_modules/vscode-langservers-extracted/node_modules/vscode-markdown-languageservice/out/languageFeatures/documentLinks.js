...
import { HrefKind, LinkDefinitionSet, MdLinkKind } from '../types/documentLink.js';
 ...
 */
const linkPattern = new RegExp(r `(?<!\\)` + // Must not start with escape
    // text
    r `(!?\[` + // open prefix match -->
    /**/ r `(?:` +
...
*/
const referenceLinkPattern = new RegExp(r `(?<![\]\\])` + // Must not start with another bracket
    r `(?:` +
    // [text][ref] or [text][]
...
export class MdLinkComputer {
        ...
        for (const match of text.matchAll(referenceLinkPattern)) {
            ...
            yield {
                ...
                source: {
                    ...
                    resource: getDocUri(document),
                    range: { start: linkStart, end: linkEnd },
                    targetRange: lsp.Range.create(translatePosition(hrefStart, { characterDelta: -1 }), translatePosition(hrefEnd, { characterDelta: 1 })),
    ...
    *#getLinkDefinitions(document, noLinkRanges) {
        ...
        for (const match of text.matchAll(definitionPattern)) {
            ...
            const refRange = { start: refStart, end: translatePosition(refStart, { characterDelta: reference.length }) };
    ...
    static #linkAttrsByTag = new Map(Array.from(htmlTagPathAttrs.entries(), ([key, value]) => [key, value.map(attr => {
...
export class MdLinkProvider extends Disposable {
    ...
    async resolveDocumentLink(inLink, token) {
    ...
    async #resolveInternalLinkTarget(linkData, token) {
    ...
    #createOpenAtPosCommand(resource, posOrRange) {
...
export function createWorkspaceLinkCache(parser, workspace) {
