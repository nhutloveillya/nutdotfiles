...
import { getLine } from '../types/textDocument.js';
...
export class MdSelectionRangeProvider {
    ...
    async #getHeaderSelectionRange(document, position, token) {
        ...
        const headerInfo = getHeadersForPosition(toc.entries.filter((entry) => entry.kind === 'header'), position);
 ...
 */
function getTableCellBoundaries(lineText) {
 ...
 */
function isTableSeparatorRow(lineText) {
 ...
 */
function createTableCellRange(lineText, cursorChar, cursorLine, parent) {
...
async function createLinkRange(document, cursorPos, parent, linkProvider, token) {
    ...
    if (rangeContains(link.source.targetRange, cursorPos)) {
        ...
        const linkDestRanges = makeSelectionRange(lsp.Range.create(translatePosition(link.source.targetRange.start, { characterDelta: 1 }), translatePosition(link.source.targetRange.end, { characterDelta: -1 })), makeSelectionRange(link.source.targetRange, fullLinkSelectionRange));
        ...
        if (link.source.isAngleBracketLink) {
            ...
            if (rangeContains(link.source.hrefRange, cursorPos)) {
                return makeSelectionRange(link.source.hrefRange, makeSelectionRange(lsp.Range.create(translatePosition(link.source.hrefRange.start, { characterDelta: -1 }), translatePosition(link.source.hrefRange.end, { characterDelta: 1 })), linkDestRanges));
            }
        }
        if (link.source.titleRange) {
        ...
        // Inside the text
        if (link.kind === MdLinkKind.Definition) {
            return makeSelectionRange(lsp.Range.create(translatePosition(link.source.range.start, { characterDelta: 1 }), translatePosition(link.source.targetRange.start, { characterDelta: -3 })), fullLinkSelectionRange);
        }
        return makeSelectionRange(lsp.Range.create(translatePosition(link.source.range.start, { characterDelta: 1 }), translatePosition(link.source.targetRange.start, { characterDelta: -1 })), fullLinkSelectionRange);
    }
}
...
function getFirstChildHeader(document, header, toc) {
    let childRange;
    if (header && toc) {
        const children = toc.filter((t) => t.kind === 'header' && rangeContains(header.sectionLocation.range, t.sectionLocation.range) && t.sectionLocation.range.start.line > header.sectionLocation.range.start.line).sort((t1, t2) => t1.declarationLocation.range.start.line - t2.declarationLocation.range.start.line);
